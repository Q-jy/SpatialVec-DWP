...
for kkk in range(len(vvv)):
    ...
    count_flag = [video.loc[i, 'flag'] for i in range(len(video))]

    # dis_edge = pd.read_csv()
    ux = [video.loc[i, 'size'] for i in range(len(video))]
    ux = sorted(ux)
    size = ux[int(len(ux)*0.25)]
    count = (video['count'] * video['flag']).quantile(0.75)
    # 服务器最大容量+
    siZe = video['size'].max()
    ...
    for i in range(len(video)):
        if video.loc[i, 'size']<size and video.loc[i, 'count']*count_flag[i]>count:
            vec.append(i)
