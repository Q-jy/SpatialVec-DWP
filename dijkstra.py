...
if __name__ == '__main__':
    ...
    for i in range(len(dis)):
        ...
        for j in range(len(dis)):
            ...
            dd[i][j] = l2
            print(i, j)
        rou.to_csv('./route/route_'+str(i)+'.csv', index=False)
    dd = pd.DataFrame(dd)
    dd.to_csv('./data/distance.csv', index=False)
    cc = pd.DataFrame(cc)
    cc.to_csv('./data/cost_rate.csv', index=False)
    ...
    station.to_csv('./data/EU455.csv', index=False)
